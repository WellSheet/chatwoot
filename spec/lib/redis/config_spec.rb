...
describe ::Redis::Config do
  context 'when single redis instance is used' do
    ...
    it 'checks for app redis config' do
      ...
      expect(app_config.keys).to match_array([:url, :password, :ssl_params, :network_timeout, :reconnect_attempts])
    ...
    it 'checks for sidekiq redis config' do
      ...
      expect(sidekiq_config.keys).to match_array([:url, :password, :size, :ssl_params, :network_timeout, :reconnect_attempts])
  ...
  context 'when redis sentinel is used' do
    ...
    it 'checks for app redis config' do
      expect(described_class.app.keys).to match_array([:url, :password, :sentinels, :ssl_params, :network_timeout, :reconnect_attempts])
    ...
    it 'checks for sidekiq redis config' do
      expect(described_class.sidekiq.keys).to match_array([:url, :password, :sentinels, :size, :ssl_params, :network_timeout, :reconnect_attempts])
